---
name: "Always Use Searx Web Search"
description: "Use the Searx Web Search MCP server for all web-related tasks to ensure current information"
alwaysApply: true
---

## Web Search & Page Reading Rule

Always use the **Searx Web Search** MCP server for any web-related tasks, including:

- ANY code or API related task, to make sure you have the latest documentation for specific libraries, modules or APIs.
- Finding the latest issues, discussions, or documentation for libraries, packages, APIs, or code snippets
- Looking up current error messages, stack traces, or recent changes in open-source projects
- Searching for up-to-date tutorials, blog posts, or news.
- Anything that requires real-time or external web information

The current date is 2026. if if your knowledge could be outdated, ALWAYS use web search.

IF you have urls in your context, but not meaningful content, therefore if you are unable to answer user's question using up to date information/documentation, use the extract_content tool on the urls you have. then generate your answer.

### MCP Server Tools
The MCP server provides two tools:

1. **web_search_urls**  
   - Use this first for most searches.  
   - Input: a clear search query (can be very specific, e.g., "python requests library latest version 2026", "fastmcp @mcp.tool decorator documentation", "pydantic v2 migration guide"). If the user input is simple, asking a simple question or demanding simple information, keep the query very short and simple. Dont add unnecessary words if you are not sure thats what the user wants or what you need.
   - Example: "python requests library latest version 2026"  
   - Returns: list of results with title, URL, domain, and short snippet.  
   - After getting results, pick the most relevant URLs and pass them to the second tool if you need the full content.

2. **extract_content**  
   - Use this when you need to read or understand the full readable content of a specific page.  
   - Input: a single URL from the search results (or any URL you already know).  
   - Returns: human-readable text with preserved links (<a href="...">some text</a>) and simplified HTML tables (<table><tr><th><td>...</td></th></tr></table>).  
   - All other HTML is flattened to clean text with appropriate newlines for paragraphs, headings, lists, code blocks, etc.  
   - Perfect for feeding detailed documentation, READMEs, issue threads, or articles to the model.
   - Dont use on youtube videos or similar non-text content.

*Do NOT start streaming your final response until all tool calls have been completed.*
*Do NOT stream tool calls as part of your final response.*
*Do not output TOOL_CALLS in your streaming response.*

## Workflow:
1. Call 'web_search_urls' tool with a brief and precise query.
2. Filter out irrelevant results based on title and preview content. Do NOT include intermediary steps in your response.
3. Check the title and preview content and compare it to the prompt to confirm relevancy. if relevant, continue.
4. Call 'extract_content' tool on the best URLs one by one to get readable content from those urls.
5. If you feel its necessary, you can also extract the contents of the relevant URLs found in a href tags in extracted contents.
6. Use the final extracted content to answer the question accurately.
7. Generate final answer using the acquired web context and all other context. Tell the user what you've done before your response.
8. Return to the original prompt and finalize your response.

Always use extract_content tool on AT LEAST one relevant url.